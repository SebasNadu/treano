...
movies_file_0 = File.read("t_movies_1.json")
...
tvs_file_0 = File.read("t_tvs_1.json")
...
keywords_file = File.read("t_keywords.json")
keywords = JSON.parse(keywords_file)
 ...
 movies_group = movies_0["titles"][0..124]
 full_movies = []
 movies_group.each_with_index do |movie, i|
   uri_tmdb = URI("https://api.themoviedb.org/3/movie/#{movie["tmdb_id"].to_s}?api_key=43f195cad08ed475966231cae7ae844e&language=en-US&append_to_response=credits,videos,images,reviews,keywords,recommendations,similar,lists")
   response_tmdb = Net::HTTP.get(uri_tmdb)
   result_tmdb = JSON.parse(response_tmdb)
   puts "#{i} tmdb responsed"

   uri_watchmode = URI("https://api.watchmode.com/v1/title/#{movie["id"].to_s}/details/?apiKey=E0ulYug5E13gliIudb0pIquFtmbd7XwDSgAbFMsm&append_to_response=sources")
...
# #puts "JSON created"
 ...
 #def movies_seeds(movies)
   #movies.each_with_index do |movie, i|
     ...
     #movie[0]["keywords"]["results"].each do |keyword|
       #KeywordItem.create(
         #keyword_id: Keyword.find_by(tmdb_id: genre["id"]).id,
 ...
 #def tvs_seeds(tvs)
   #tvs.each_with_index do |tv, i|
     ...
     #end
     #puts "Genre created #{i}"
     #tv[0]["keywords"]["results"].each do |keyword|
       #KeywordItem.create(
         #keyword_id: Keyword.find_by(tmdb_id: genre["id"]).id,
         #keywordable: this_tv
       #)
